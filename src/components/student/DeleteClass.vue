  ...
  <div>
    <div>
      <el-breadcrumb separator="/" class="breadcrumb">
        ...
        <el-breadcrumb-item :to="{ path: '/deleteClass' }"></el-breadcrumb-item>
      </el-breadcrumb>
      <div style="margin-left: 100px;margin-top: 50px">
        <div style="margin-left: 50px; width: 80%">
          ...
          <el-divider content-position="center">退选</el-divider>
          <el-table
              :data="classList"
              border
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-button  type="primary" size="mini" plain @click.native.prevent="deleteRow(scope.$index, classList)" @click="choiceClass(scope.row)">退选</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  methods: {
    ...
    async choiceClass(data) {
      ...
      const {data: response} = await this.$http.get('http://localhost:8880/user/DeleteClassInfo', {params: {className: className}});
