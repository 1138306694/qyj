  ...
  <div>
    <div>
      <el-breadcrumb separator="/" class="breadcrumb">
        ...
        <el-breadcrumb-item :to="{ path: '/rebuild'}">重修</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div style="margin-left: 100px;margin-top: 50px">
      <div style="width: 85%">
        <el-form ref="searchForm" :inline="true" :rules="rules" :model="searchFrom">
          ...
          <el-form-item>
            <el-button type="primary" @click="getRebuildInfo">检索</el-button>
          </el-form-item>
        </el-form>
        <el-divider content-position="center">重修课程表</el-divider>
        <el-table
            :data="rebuildInfo"
            border
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button ref="btn"  type="primary" size="mini" plain @click="rebuild(scope.row)">重修报名</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    async getRebuildInfo(){
      await this.$refs.searchForm.validate(async (valid) => {
        ...
        if (valid){
          ...
          const {data: response} = await this.$http.post('http://localhost:8880/user/getRebuildInfo', {
          ...
          this.rebuildInfo = response.data;
        }
      })
    },
    async rebuild(data){
      ...
      //重修的选课会带一个pickType=1
      const {data: response} = await this.$http.post('http://localhost:8880/user/studentPickClass',  {"className":className,"pickType": 1});
      if(response.code === 200){
        this.$message.info("重修选课成功")
      }
